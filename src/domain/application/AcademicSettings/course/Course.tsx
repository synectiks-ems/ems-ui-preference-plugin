...
import { SAVE_COURSE, GET_COURSE_LIST} from '../../_queries';
...
class CourseObj {
  ...
  courseDuration: any; 
  courseType: any;
  yearOrSemesterType: any;
  totalFee: any;
  yearlyFee: any;
  perSemesterFee: any;
  status: any;

  constructor(name: any, description: any, courseDuration: any, courseType: any, yearOrSemesterType: any, totalFee: any, yearlyFee: any, perSemesterFee: any, branchId: any, departmentId: any, status: any) {
    this.name = name;
      ...
      this.state = {
          ...
          modelHeader: "",
                ...
                },
                {
                  ...
                  {
                    label: "Year/Semester Course",
                    key: 'yearOrSemesterType',
                    isCaseInsensitive: false,
                  },
                  {
                    label: "Total Fee",
                    ...
                    renderCallback: (value: any, alert: any) => {
                      return <td>
                        <div className="d-inline-block">
                          <button className="btn btn-primary" onClick={e => this.showDetail(e, true, alert, "Edit Course")}>
                            {/* <i onClick={e => this.showDetail(e,true,alert, "Edit Course")} className="fa fa-edit"></i> */}
                                          Edit
                                        </button>
                        </div>
                      </td>
    ...
    async componentDidMount() {
      ...
      getCourse = async () => {
        const { data } = await this.props.client.query({
          ...
          const data=res.data;
          console.log("Course data :::", data.getCourseList);
      ...
      render() {
        const { columns, courseList, departmentList, branchList, isModalOpen, courseObj, modelHeader, errorMessage, successMessage } = this.state;
        return (
    
          <section className="customCss">
            <Modal isOpen={isModalOpen} className="react-strap-modal-container" style={{height:"1000px", overflow:"auto"}}>
            ...
            <Table valueFromData={{ columns: this.state.columns, data: courseList }} perPageLimit={6} visiblecheckboxStatus={true} tableClasses={{ table: "alert-data-tabel", tableParent: "alerts-data-tabel", parentClass: "all-alert-data-table" }} searchKey="name" showingLine="Showing %start% to %end% of %total%" />
